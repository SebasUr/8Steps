...
function Steps() {
      ...
      scrollInstance.current.on("scroll", () => {
        blocks.forEach((block) => {
          ...
          if (section.getBoundingClientRect().left <= block.offsetWidth * 2) {
            ...
            window.innerWidth - block.offsetWidth * 2
          ) {
  ...
  useEffect(() => {
    // Desactivar el scroll vertical solo en esta página
    document.body.style.overflow = "hidden";

    // Limpiar cuando el componente se desmonte
    return () => {
      document.body.style.overflow = ""; // Restaurar comportamiento original
    };
  }, []);
  ...
  const saveToProfile = async () => {
    ...
    const newTrajectory =
      trajectories === null ? [jobs] : [...trajectories, jobs];
    await api
      .patch("/users/api/user/profile/", { last_trajectory: newTrajectory })
      .then((response) => response.data)
      .then((data) => {
        ...
        <div className="blocks">
          <div className="navbar-steps">
            ...
            <div className="navbar-steps-download">
                <img src="/download.png" onClick={saveToProfile} alt="Descargar" className="logo" />
            </div>
                          ...
                          <div className="job-card-container">
                            {job.jobs_list.map((job, jobIndex) => (
                              ...
                              >
                                ...
                                {job.city && (
                                  <div className="job-location">{job.city}</div>
                                )}
                              </a>
                            ))}
                          </div>
